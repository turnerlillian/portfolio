...
import { ArrowRight, Palette, Code, Users, BookOpen } from "lucide-react";
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {services.map((service, index) => (
                <Card key={index} className={`card-contemporary ${service.variant} group`}>
                  <CardHeader className="p-8">
                    <div className="mb-4 text-current opacity-80 group-hover:scale-110 transition-transform">
                      {service.icon}
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              {features.map((feature, index) => (
                <div key={index} className="group">
                  <div className={`text-5xl md:text-6xl font-black mb-4 ${feature.color} group-hover:scale-110 transition-transform`}>
                    {feature.number}
                  </div>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <Link to="/ux-case-studies">
                <Button size="lg" className="btn-contemporary min-w-[200px]">
                  View My Work
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </Link>
              <Link to="/contact">
                <Button size="lg" className="btn-olive min-w-[200px]">
                  Get In Touch
                </Button>
              </Link>
            </div>
